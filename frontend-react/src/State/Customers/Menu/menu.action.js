...
export const createMenuItem = ({ menu, jwt }) => {
  return async (dispatch) => {
    ...
    try {
      console.log("Sending request to create menu item:", menu);
      ...
      console.log("Created menu item response data:", data);
      dispatch(createMenuItemSuccess(data));
    } catch (error) {
      console.log("catch error ", error);
      dispatch(createMenuItemFailure(error.response?.data || error.message));
    }
  };
};
  ...
  return async (dispatch) => {
    ...
    try {
      const { data } = await api.get(
        `/api/food/restaurant/${reqData.restaurantId}?vegetarian=${reqData.vegetarian}&nonveg=${reqData.nonveg}&seasonal=${reqData.seasonal}&foodCategory=${reqData.foodCategory}`,
        {
          headers: {

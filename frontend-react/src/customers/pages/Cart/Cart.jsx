...
import { findCart, deleteAddress } from "../../../State/Customers/Cart/cart.action";
...
const Cart = () => {
  ...
  const handleSubmit = (values, { resetForm }) => {
    const totalAmount = cartTotal(cart.cartItems) + 3; // Calculate the total amount
    const data = {
  ...
  const createOrderUsingSelectedAddress = (deliveryAddress) => {
    const totalAmount = cartTotal(cart.cartItems) + 3;
    const data = {
      jwt: localStorage.getItem("jwt"),
      order: {
        restaurantId: cart.cartItems[0].food.restaurant.id,
  ...
  const handleDeleteAddress = (addressId) => {
    dispatch(deleteAddress({ addressId, jwt: localStorage.getItem("jwt") }));
  };
          ...
          <section className="lg:w-[30%] space-y-6 lg:min-h-screen pt-10">
            ...
            <div className="billDetails px-5 text-sm">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-gray-400">
                  <p>Total Pay</p>
                  <p>€{cartTotal(cart.cartItems) + 3}</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="lg:w-[70%] flex justify-center px-5 pb-10 lg:pb-0">
            <div>
              ...
              <div className="flex gap-5 flex-wrap justify-center">
                {auth.user?.addresses
                  .filter((item) => !item.deleted) // Filter out deleted addresses
                  .map((item, index) => (
                    <AddressCard
                      key={index}

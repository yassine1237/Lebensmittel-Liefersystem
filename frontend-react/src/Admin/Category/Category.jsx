...
import { getRestaurantById, getRestaurantsCategory,deleteCategory } from '../../State/Customers/Restaurant/restaurant.action';
import { Box, Card, CardHeader, IconButton, Modal, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Button } from '@mui/material';
...
const Category = () => {
    ...
    useEffect(() => {
        if (restaurant?.id) {
            dispatch(getRestaurantsCategory(restaurant.id));
        }
    }, [restaurant, dispatch]);
                        ...
                        <TableBody>
                            {restaurant.categories.map((item, index) => (
                                ...
                                >
                                    ...
                                    <TableCell className="">
                                        <IconButton className="text-red-500" onClick={() => handleDeleteCategory(item.id)}>
                                            <Delete />
                                        </IconButton>
                                    </TableCell>
                                </TableRow>
